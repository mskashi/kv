...
#define ODE_MAFFINE0_HPP

// ODE using Affine and Mean Value Form (obsoleted)
...
int
ode_maffine0(F f, ub::vector< affine<T> >& init, const interval<T>& start, interval<T>& end, ode_param<T> p = ode_param<T>() , ub::matrix< interval<T> >* mat = NULL, ub::vector< psa< interval<T> > >* result_psa = NULL)
{
	int n = init.size();
	int i, j;
	...
	while (true) {
		...
		p.order++;
		if (p.verbose == 1) {
			std::cout << "ode_maffine0: increase order: " << p.order << "\n";
		}
	}
