...
/*
 * take from
 *  https://twitter.com/dannchu/status/1753393768915726786
 * example including min function
 */ 
...
struct Chiba {
	...
	template <class T> T operator() (const ub::vector<T>& x){
		...
		tmp1 = 1;
		for (i=0; i<n; i++) tmp1 *= x(i);
		tmp2 = 1;
		for (i=0; i<n; i++) tmp2 *= 1 - x(i) * x(i);
...
int main()
{
	...
	result = minimize_nd(I, Func(), 1e-5);
	p = result.begin();
	while (p != result.end()) {
		std::cout << *(p++) << "\n";
	}
	...
	result = minimize_nd(I, Func(), 10000);
	p = result.begin();
	while (p != result.end()) {
		std::cout << *(p++) << "\n";
	}
	...
	v = minimize_nd_value(I, Func(), 10000);
	std::cout << v << "\n";

	std::cout << "calculate intervals maximizing Func\n";
	result = maximize_nd(I, Func(), 10000);
	p = result.begin();
	while (p != result.end()) {
		std::cout << *(p++) << "\n";
	}
	...
	v = maximize_nd_value(I, Func(), 10000);
	std::cout << v << "\n";

	std::cout << "calculate intervals minimizing Func2\n";
	result2 = minimize_nd(itv(1,10), Func2(), 10000);
	p2 = result2.begin();
	...
	v = minimize_nd_value(itv(1,10), Func2(), 10000);
	std::cout << v << "\n";

	std::cout << "calculate intervals maximizing Func2\n";
	result2 = maximize_nd(itv(1,10), Func2(), 10000);
	p2 = result2.begin();
	while (p2 != result2.end()) {
		std::cout << *(p2++) << "\n";
	}
	...
	v = maximize_nd_value(itv(1,10), Func2(), 10000);
	std::cout << v << "\n";
	...
	result = minimize_nd(I, NTTData(), 1e-3);
	p = result.begin();
	while (p != result.end()) {
		std::cout << *(p++) << "\n";
	}
	...
	I.resize(2);
	for (i=0; i<I.size(); i++) I(i) = itv(0., 10.);
	result = minimize_nd(I, Levy(), 1e-5);
	p = result.begin();
	while (p != result.end()) {
		std::cout << *(p++) << "\n";
	}
	...
	result = minimize_nd(I, Griewank(n), 1e-5);
	p = result.begin();
	while (p != result.end()) {
		std::cout << *(p++) << "\n";
	}
	...
	n = 4;
	I.resize(n);
	for (i=0; i<I.size(); i++) I(i) = itv(0., 1.);
	v = maximize_nd_value(I, Chiba(n), 1e-4);
	std::cout << v << "\n";
}
